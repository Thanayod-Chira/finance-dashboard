...
router.get('/', async (req, res) => {
  const result = await pool.query('SELECT * FROM subscriptions ORDER BY next_billing_date');
...
router.post('/', async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO subscriptions (name, amount, billing_cycle, next_billing_date, category)
     VALUES ($1,$2,$3,$4,$5) RETURNING *`,
    [name, amount, billingCycle, nextBillingDate, category || null]
  );
...
router.put('/:id', async (req, res) => {
  ...
  const result = await pool.query(
    ...
    [name, amount, billingCycle, nextBillingDate, category, isActive, req.params.id]
  );
